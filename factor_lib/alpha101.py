...
def get_alpha(df):
        ...
        df['alpha056']=stock.alpha056()
        df['alpha057']=stock.alpha057()
        df['alpha058']=stock.alpha058()
        df['alpha059']=stock.alpha059()
...
class Alphas(object):
    def __init__(self, df_data):
        ...
        self.returns = df_data['close'].pct_change().fillna(0)
        self.open_time = df_data.index
        #self.vwap = (df_data['S_DQ_AMOUNT']*1000)/(df_data['S_DQ_VOLUME']*100+1) 
        self.vwap = df_data['volume']
